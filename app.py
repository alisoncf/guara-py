...
def create_app(config_name=None):
    ...
    app.config['SWAGGER'] = {
        ...
        'components': {
            ...
            'schemas': { # ADICIONADO AQUI
                'MediaSparqlInfo': {
                    'type': 'object',
                    'properties': {
                        'media_uri': {'type': 'string', 'format': 'uri', 'example': 'http://example.org/media/componenteA.jpg', 'description': 'URI da mídia conforme registrada no SPARQL.'},
                        'objeto_associado_uri': {'type': 'string', 'format': 'uri', 'example': 'http://localhost:3030/meu_dataset#objfisico-789-uuid-012', 'description': 'URI do objeto ao qual a mídia está associada.'}
                    },
                ...
                'ArquivoCombinadoInfo': {
                    'type': 'object',
                    'properties': {
                        'nome_arquivo_local': {'type': 'string', 'example': 'componenteA.jpg', 'description': 'Nome do arquivo encontrado localmente.'},
                        'uri_sparql_correspondente': {'type': 'string', 'format': 'uri', 'nullable': True, 'example': 'http://example.org/media/componenteA.jpg', 'description': 'URI da mídia no SPARQL que corresponde ao arquivo local (se houver).'},
                        'presente_localmente': {'type': 'boolean', 'description': 'Indica se o arquivo foi encontrado na pasta de uploads.'},
                        'presente_sparql': {'type': 'boolean', 'description': 'Indica se uma URI correspondente foi encontrada no SPARQL.'}
                    },
    ...
    if environment == 'development':
        ...
        if use_ssl:
            if os.path.exists(cert_path) and os.path.exists(key_path):
                ...
                app.logger.warning(f"Certificados SSL não encontrados em '{cert_path}' ou '{key_path}'. A executar sem SSL.")
                app.run(host='0.0.0.0', port=port, debug=False) 
        else:
