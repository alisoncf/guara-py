...
#
# obj:colecao foi REMOVIDA desta consulta de propósito: como um objeto pode
# ter mais de uma coleção associada (domínio ampliado para :ObjetoDigital,
# sem restrição de cardinalidade), incluir ?colecao no SELECT com um
# OPTIONAL simples gera produto cartesiano — o SPARQL retorna uma linha
# PARA CADA COMBINAÇÃO de coleção, duplicando também título/descrição/
# resumo daquela linha. Isso já causou um bug real: objetos com múltiplas
# coleções apareciam duplicados no acervo comparado, incluindo uma
# comparação espúria do objeto consigo mesmo (similaridade 1.0). Como
# 'colecao' não é usada no texto comparado (só título+descrição+resumo),
# a solução mais simples é não buscá-la aqui. Se um dia for necessária
# (ex. para filtrar recomendações por categoria de acervo), usar
# GROUP_CONCAT(?colecao; separator="|") com GROUP BY pelos demais campos,
# nunca um OPTIONAL simples.
SPARQL_QUERY_OBJETOS = """
PREFIX dc: <http://purl.org/dc/terms/>
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX obj: <http://guara.ueg.br/ontologias/v1/objetos#>

SELECT ?objeto ?tipo ?titulo ?descricao ?resumo
WHERE {
